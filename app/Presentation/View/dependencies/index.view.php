...
<x-layout>
    ...
    <form class="filter-bar" method="GET" action="/dependencies">
        <input type="text" name="q" placeholder="<?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.search_dependencies')) ?>" value="<?= htmlspecialchars(
            $criteria->query ?? '',
        ) ?>">

        <select name="source_component_id"><option value=""><?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.all_sources')) ?></option><?php foreach ($components as $component): ?><option value="<?= $component->id() ?>" <?= $criteria->sourceComponentId === $component->id()
            ? 'selected'
            : '' ?>><?= htmlspecialchars($component->name()) ?></option><?php endforeach; ?></select>
        <select name="target_component_id"><option value=""><?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.all_targets')) ?></option><?php foreach ($components as $component): ?><option value="<?= $component->id() ?>" <?= $criteria->targetComponentId === $component->id()
            ? 'selected'
            : '' ?>><?= htmlspecialchars($component->name()) ?></option><?php endforeach; ?></select>
        <select name="dependency_type_id"><option value=""><?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.all_types')) ?></option><?php foreach ($dependencyTypes as $type): ?><option value="<?= $type['id'] ?>" <?= $criteria->dependencyTypeId
        === (int) $type['id']
            ? 'selected'
            : '' ?>><?= htmlspecialchars($type['name']) ?></option><?php endforeach; ?></select>
        <select name="protocol_id"><option value=""><?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.all_protocols')) ?></option><?php foreach ($protocols as $protocol): ?><option value="<?= $protocol['id'] ?>" <?= $criteria->protocolId
        === (int) $protocol['id']
            ? 'selected'
            : '' ?>><?= htmlspecialchars($protocol['name']) ?></option><?php endforeach; ?></select>
        <select name="status_id"><option value=""><?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.all_statuses')) ?></option><?php foreach ($statuses as $status): ?><option value="<?= $status['id'] ?>" <?= $criteria->statusId
        === (int) $status['id']
            ? 'selected'
            : '' ?>><?= htmlspecialchars($status['name']) ?></option><?php endforeach; ?></select>
        <select name="criticality_id"><option value=""><?= htmlspecialchars(\App\Shared\Support\Translator::translate('filter.all_criticalities')) ?></option><?php foreach ($criticalityLevels as $level): ?><option value="<?= $level['id'] ?>" <?= $criteria->criticalityId
        === (int) $level['id']
            ? 'selected'
            : '' ?>><?= htmlspecialchars($level['name']) ?></option><?php endforeach; ?></select>
        ...
        === (int) $object['id']
            ? 'selected'
            : '' ?>><?= htmlspecialchars($object['name']) ?></option><?php endforeach; ?></select>
    ...
    <div class="table-wrapper">
        <table class="data-table">
            <thead><tr><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate('table.name')) ?></th><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                'table.source',
            )) ?></th><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate('table.target')) ?></th><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                'table.data',
            )) ?></th><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate('table.frequency')) ?></th><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                'table.quality',
