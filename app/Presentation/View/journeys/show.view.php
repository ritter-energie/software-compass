...
<x-layout>
    ...
    <section class="panel">
        <div class="section-header"><h3><?= htmlspecialchars(\App\Shared\Support\Translator::translate('journeys.steps')) ?></h3><a class="button-secondary" href="/journeys/<?= $journey->id() ?>/steps/create">+ <?= htmlspecialchars(\App\Shared\Support\Translator::translate(
            'journeys.add_step',
        )) ?></a></div>
        <?php foreach ($steps as $step): ?>
            <article class="panel">
                <div class="section-header"><h4><?= $step->sortOrder() ?>. <?= htmlspecialchars($step->name()) ?></h4><div class="actions"><a href="/journey-steps/<?= $step->id() ?>/edit"><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                    'journeys.edit_step',
                )) ?></a><form method="POST" action="/journey-steps/<?= $step->id() ?>/delete" data-confirm="<?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                    'journeys.confirm_delete_step',
                )) ?>"><?= \App\Shared\Support\Csrf::input() ?><button class="button-danger" type="submit"><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                    'common.delete',
                )) ?></button></form></div></div>
                ...
                <table class="data-table"><thead><tr><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate('table.component')) ?></th><th><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
                    'table.role',
    ...
    <section class="panel"><h3><?= htmlspecialchars(\App\Shared\Support\Translator::translate('journeys.diagram')) ?></h3><pre class="mermaid"><?= htmlspecialchars($mermaid) ?></pre><details><summary><?= htmlspecialchars(\App\Shared\Support\Translator::translate(
        'diagrams.mermaid_source',
    )) ?></summary><pre><?= htmlspecialchars($mermaid) ?></pre></details></section>
</x-layout>
